...
def set_img_color(colors, background, img, gt, show255=True, weight_foreground=0.55):
    origin = np.array(img)
    img = img.transpose(0,2,3,1)  # img: [b,h,w,c]
    for i in range(len(colors)):
        if i != background:
            img[np.where(gt == i)] = colors[i]
    if show255:
        img[np.where(gt == 255)] = [86, 62, 67]
    # img = img.transpose(0,3,1,2) # 要らない, cv2は[h,w,c],bgrの順番を求めている.
...
def show_prediction(colors, background, img, pred, weight_foreground=1):
    ...
    im = set_img_color(colors, background, im, pred, weight_foreground=weight_foreground)
    final = np.array(im)
    return final
