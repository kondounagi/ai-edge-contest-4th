...
class Evaluator(object):
    def __init__(self, dataloader, dataset, class_num, image_mean, image_std, network,
    ...
    def single_process_evaluation(self):
        all_results = []
        valid_class = np.array([0,6,11,13])
        with torch.no_grad():
            for i, dd in enumerate(self.dataloader):
                results_dict = self.func_per_iteration(dd[0], self.devices[0])   # single-gpuを想定
...
#         return result_line, mIoU
    ...
    def whole_eval(self, img, output_size, input_size=None, device=None):
        ...
        pred = self.val_func_process(img, device)   # pred.shape = [b,c,h,w]
        pred = pred.permute(0,2,3,1)
        pred = pred.cpu().numpy()
        pred = pred.argmax(3)     # pred \in [0~18]^(b*h*w) 圧倒的にroadが多い
...
#         pred = self.val_func_process(img, device)   # [c,h,w]
#         if input_size is not None:
...
#         return pred
    ...
    def val_func_process(self, input_data, device=None):
        ...
        self.val_func.eval()
        self.val_func.to(input_data.device)
        with torch.no_grad():
