...
def main():
    ...
    train_loader = get_train_loader(config, Dataset, "train")
    ...
    for idx, arch_idx in enumerate(config.arch_idx):
        ...
        flops, params = prof(model, inputs=(torch.randn(1, 3, 1024, 2048),), verbose=False)
        ...
        if config.is_eval:
            ...
            # train student
            if len(config.arch_idx) == 2:
                if arch_idx == 1 and config.pretrained_path != None:
                    partial = torch.load(os.path.join(config.pretrained_path, "weights%d.pt"%arch_idx))
                    state = model.state_dict()
                    pretrained_dict = {k: v for k, v in partial.items() if k in state}
                    state.update(pretrained_dict)
                    model.load_state_dict(state)
        ...
        # modelはここで完成しているので, これを書き出す. batch_size=3
#         if arch_idx == 1:
#             model.eval()
#             dummy_input = torch.randn(3, 3, 320, 544, device='cuda')
#             input_names = ["input_array"]
#             output_names = ["final_output"]
#             torch.onnx.export(model, dummy_input, "finetuned_model/FasterSeg1228_0375_student.onnx", verbose=True, input_names=input_names, output_names=output_names, opset_version=11)
#             print("export student model to onnx")
            
        evaluator = SegEvaluator(val_loader, Dataset, config.num_classes, config.image_mean,
        ...
        tester = SegTester(test_loader, Dataset, config.num_classes, config.image_mean,
                                 config.image_std, model, config.eval_scale_array, config.eval_flip, 0, out_idx=0, config=config,
    ...
    for epoch in tbar:
        ...
        # adjust_learning_rate(base_lr, 0.998, optimizer, epoch+1, config.nepochs)
        # finetune, 200epoch
        adjust_learning_rate(base_lr, 0.977, optimizer, epoch+1, config.nepochs)
        # finetune, 400 epoch
        # adjust_learning_rate(base_lr, 0.989, optimizer, epoch+1, config.nepochs)
        # finetune, 800 epoch
        # adjust_learning_rate(base_lr, 0.994, optimizer, epoch+1, config.nepochs)
    ...
    for minibatch in train_loader:
        ...
        for idx, arch_idx in enumerate(config.arch_idx):
            ...
            if arch_idx == 0 and len(models) > 1:
                ...
                logits_list.append(logits8)
                loss = loss + criterion(logits8, target)          #ohem_criterion → loss_opr.py
        ...
        logger.add_scalar('loss/train', loss+loss_kl, epoch*2000)
...
#         pbar.set_description("[Step %d/%d]"%(step + 1, config.niters_per_epoch) + description)
...
# eval teacher and student network
...
# # eval only student network
# def infer(models, evaluators, logger):
#     models[1].eval()
#     _, mIoU = evaluators[1].run_online()
#     return mIoUs
