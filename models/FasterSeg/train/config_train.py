...
"""Train Config"""
C.lr = 0.01 * 0.1 # learning rate for updating supernet weight (NOT arch params)
C.momentum = 0.9 # SGD momentum
C.weight_decay = 5e-4 # SGD weight decay
C.nepochs = 200 # how many epochs to train
C.niters_per_epoch = 2975 // 12  # 1000 # how many batches per epoch
...
if C.mode == "teacher":
    ...
    C.pretrained_path = "fasterseg" # finetune Cityscapes_pre-trained_model by signate
    ...
    C.load_path = "fasterseg" # path to the searched directory
    C.teacher_path = "train-1216x1920_teacher_batch12_nepoch200_signate" # where to load the pretrained teacher's weight
    C.pretrained_path = "fasterseg"
...
C.eval_path = "train-448x708_student_batch12_nepoch200_signate" # path to pretrained directory to be evaluated
